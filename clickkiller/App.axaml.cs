...
public partial class App : Application
{
    ...
    public static string GetAppPath()
    {
        ...
        string productionPath = Path.Combine(appDataPath, appFolder);
    ...
    private void SeedDatabase()
    {
        ...
        if (IsDebugMode())
        {
            ...
            string debugPath = AppDomain.CurrentDomain.BaseDirectory;
            string dbName = "issues.db";
            string prodDbPath = Path.Combine(productionPath, dbName);
            string debugDbPath = Path.Combine(debugPath, dbName);

            if (File.Exists(prodDbPath) && !File.Exists(debugDbPath))
            {
                try
                {
                    Logger.LogInformation("Seeding debug database from production: {prodDbPath} -> {debugDbPath}", prodDbPath, debugDbPath);
                    File.Copy(prodDbPath, debugDbPath);
                }
                catch
                {
                    // Ignore copy errors (e.g. file in use)
                }
            }
        }
    }
